...
def parse_clans_edgefile(f):
  """
  Takes an a file-like object of an 'edge file'
  from "Save blast matrix pP-values" of export CLANS.

  Returns a list of edge tuples and a dictionary of
  sequence groups.
  """
  edges = []
  seqgroups = {}
  ...
  return edges, seqgroups

def parse_clans_nodefile(f):
  """
  Takes an file-like object of a CLANS
  node position file, exported via
  "Save 2d graph data".

  Returns a list of tuples (id, name, x, y).
  """
  nodes = []
...
if __name__ == "__main__":
  ...
  #####################################
  edges, seqgroups = parse_clans_edgefile(f)
  f.close()
